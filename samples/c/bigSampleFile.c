...
Yığın *yeniYığın()
{
    ...
    if(bu == GEÇERSİZ || bu->sıra == GEÇERSİZ)
    {
...
bool Yığınİtle(Yığın *bu, kesirli eleman)
{
    ...
    {
        bu->sıra[bu->boy++] = eleman; //Add the element to the stack and then increase the size of the stack.
        cevapVer doğru;
    };
...
boş YığınTemizle(Yığın *bu)
{
    ...
    bu->sıra = bellekAyır(u(float) * bu->kapasite); //Allocate new memory to the array.
    eğer (bu->sıra == GEÇERSİZ)
    {
        yazf("Memory Allocation Failed");
        YığınParçala(bu);
